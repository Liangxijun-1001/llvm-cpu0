; RUN: llc  -march=cpu0el -mcpu=cpu032I -relocation-model=pic -O3 < %s | FileCheck %s -check-prefix=16_h
; RUN: llc  -march=cpu0el -mcpu=cpu032I -relocation-model=pic -O3 < %s | FileCheck %s -check-prefix=16_b
...
; 16_b: test:
; 16_h: test:
; 16_b:	sb	${{[0-9]+|t9}}, {{[0-9]+|t9}}(${{[0-9]+|t9}})
; 16_b: lb      ${{[0-9]+|t9}}, {{[0-9]+|t9}}($sp)
; 16_h:	sh	${{[0-9]+|t9}}, {{[0-9]+|t9}}(${{[0-9]+|t9}})
; 16_h: lh      ${{[0-9]+|t9}}, {{[0-9]+|t9}}($sp)
}
